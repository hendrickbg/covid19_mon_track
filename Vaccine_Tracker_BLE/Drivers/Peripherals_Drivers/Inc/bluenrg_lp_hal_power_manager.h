/**
  ******************************************************************************
  * @file    bluenrg_lp_hal_power_manager.h
 ...
 *     Wakeup is possible from GPIOs (PA0 to PA15 and PB0 to PB11), RTC, IWDG, MR_BLE and the Hal Virtual Timers.
...
/** @brief Wakeup source BLE Host Timer Mask */
#define WAKEUP_BLE_HOST_TIMER LL_PWR_EWS_BLEHOST

/** @brief Wakeup source BLE Mask */
...
/** 
 * @brief This function allows to put the BlueNRG-LP in low power state.
 *
 * This function allows to enter a desired power save level that is negotiated between BTLE stack needs, CPU Host Timer needs and application needs.
...
/** 
 * @brief This function allows to put the BlueNRG-LP in shutdown.
 *
 * This function allows to put the BlueNRG-LP in shutdown mode, this is the least power consuming mode.
 ...
 * 
 * @retval ERROR if for some reason the BlueNRG-LP not allow the Shutdown mode.
 *
 ...
 *       The BluenRG-LP device during the DEEPSTOP period has the GPIO peripherals disabled.
